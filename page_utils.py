...
class Pagination(object):
    ...
    # @property
    # def items(self):
    #     return Access.query.order_by('-id').paginate(self.current_page, self.per_page_count, error_out=False).items
    ...
    def page_html(self):
        ...
        if self.params["page"] < 1:
            previous_page = '<li class="disabled"><a href="%s?%s" aria-label="Previous">上一页</span></a></li>'.decode(
                "utf-8") % (self.base_url, urlencode(self.params))
        else:
            previous_page = '<li><a href = "%s?%s" aria-label = "Previous" >上一页</span></a></li>'.decode("utf-8") % (
